...
compute_metrics <- function(TABLE){
    ...
    face_h <- euc_dist(TABLE, "Gn", "N") # facial height
    lower_f <- euc_dist(TABLE, "Gn", "Sn") # lower face
    maxilla <- euc_dist(TABLE, "Um", "Sn") # maxilla
    mandible <- euc_dist(TABLE, "Gn", "Um") #mandible



    lower_face <- (lower_f / face_h) * 100
    max_st <- (maxilla / face_h) * 100
    mand_st <- (mandible / face_h) * 100
...
exp_tbl <- files[c(5:8)] %>%
    ...
    map(read_csv)
## landmark list
pfla_ldmk <- as.character(c(9, 63, 34, 28))
pfla_lbl <- c("Id",
              "Gn-X", "Gn-Y",
              "Um-X", "Um-Y",
              "Sn-X", "Sn-Y",
...
## Plot results ------------------------------------------
## modify tables for added convenience
